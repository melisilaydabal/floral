...
class SVM():
    ...
    def _calculate_gram_matrix(self, X1, X2):
        ...
        if self.kernel in ['linear', 'linear_smooth']:
            ...
            return correction_matrix * rbf_kernel_matrix
        else:
            raise ValueError(
    ...
    def approximate_kernel(self, gamma, num_components, X_train, X_val, X_test):
        ...
        if self.kernel in ['rbf', 'rbf_smooth']:
            rbf_sampler = RBFSampler(gamma=gamma, n_components=num_components, random_state=42)
            X_train_approx = rbf_sampler.fit_transform(X_train)
            X_val_approx = rbf_sampler.transform(X_val)
    ...
    def fit(self, X, y):
        ...
        p = cvxopt.matrix(np.ones(num_samples) * -1)
        ...
        for n in range(len(self.sv_alphas)):
        ...
        if self.kernel == 'linear':
            self.w = np.zeros(len(self.sv_alphas))
            self.w = ((self.sv_y * self.sv_alphas).T @ self.sv.numpy())
        else:
            self.w = None

    def fit_adversarial(self, X, y, y_adv, is_set_flip, is_active_learn):
        y = y.numpy()
        num_samples, num_features = X.shape
        ...
        if self.C is None or self.C == 0:
            ...
            G = cvxopt.matrix(np.vstack((tmp1, tmp2)))
            tmp1 = np.zeros(num_samples)
            tmp2 = np.ones(num_samples - len(y_adv)) * self.C
            tmp3 = np.ones(len(y_adv)) * self.C * self.reg_param
            if self.sample_weights is not None:
                # Weighted SVM training
                tmp2 = tmp2 * self.sample_weights.numpy()
            ...
            if self.flip_indices is not None:
                # Concat to one list, because they may poison the same indices
                poisoned_indices = list(set(item for sublist in self.flip_indices for item in sublist))
                tmp4 = np.hstack((tmp2, tmp3))
                tmp4[poisoned_indices] = self.C * self.reg_param
                h = cvxopt.matrix(np.hstack((tmp1, tmp4)))
    ...
    def attack_fit(self, X, y):
        """Generates an attacker-SVM decision boundary that maximizes the objective"""
        y = y.numpy()
        num_samples, num_features = X.shape
    ...
    def _project(self, X):
        ...
        if self.w is not None:
            ...
            for i in range(len(X)):
                # Compute the decision function using vectorized operations
